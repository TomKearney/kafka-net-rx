...
namespace KafkaNet
{
    ...
    /// </summary>
    public abstract class ConsumerBase : IMetadataQueries, IDisposable
    {
        ...
        {
            get { return _partitionPollingIndex.Count; }
        }
        ...
        {
            return
                _partitionOffsetIndex.Select(x => new OffsetPosition {PartitionId = x.Key, Offset = x.Value}).ToList();
        }

        internal void EnsurePartitionPollingThreads()
        {
            try
            {
                if (Interlocked.Increment(ref _ensureOneThread) == 1)
                {
                    ...
                    if (topic.Count <= 0)
                        throw new ApplicationException(string.Format("Unable to get metadata for topic:{0}.",
                            _options.Topic));
                    _topic = topic.First();
                    ...
                    foreach (var partition in _topic.Partitions)
                    {
                        ...
                        if (_options.PartitionWhitelist.Count == 0 ||
                            _options.PartitionWhitelist.Any(x => x == partitionId))
                        {
            ...
            {
                _options.Log.ErrorFormat("Exception occured trying to setup consumer for topic:{0}.  Exception={1}",
                    _options.Topic, ex);
            }
            finally
            {
        ...
        protected ConsumerOptions Options => _options;
        ...
        private Task ConsumeTopicPartitionAsync(string topic, int partitionId)
        {
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    _options.Log.DebugFormat("Consumer: Creating polling task for topic: {0} on parition: {1}", topic,
                        partitionId);
                    while (_disposeToken.IsCancellationRequested == false)
                    {
                        try
                        {
                            ...
                            if (responses.Count > 0)
                            {
                                ...
                                if (response != null && response.Messages.Count > 0)
                                {
                                    ...
                                    {
                                        ConsumeMessage(message, _disposeToken.Token);
                        ...
                        {
                            _options.Log.ErrorFormat(
                                "Exception occured while polling topic:{0} partition:{1}.  Polling will continue.  Exception={2}",
                                topic, partitionId, ex);
                        }
                    }
                }
                finally
                {
                    _options.Log.DebugFormat("Consumer: Disabling polling task for topic: {0} on parition: {1}", topic,
                        partitionId);
                    Task tempTask;
            ...
            switch ((ErrorResponseCode) response.Error)
            {
                ...
                case ErrorResponseCode.OffsetOutOfRange:
                    throw new OffsetOutOfRangeException(
                        "FetchResponse indicated we requested an offset that is out of range.  Requested Offset:{0}",
                        request.Offset) {FetchRequest = request};
                ...
                case ErrorResponseCode.NotLeaderForPartition:
                    throw new InvalidMetadataException(
                        "FetchResponse indicated we may have mismatched metadata.  ErrorCode:{0}", response.Error)
                    {
        ...
        private void FixOffsetOutOfRangeExceptionAsync(Fetch request)
        {
            _metadataQueries.GetTopicOffsetAsync(request.Topic)
                .ContinueWith(t =>
                {
                    ...
                    {
                        _options.Log.ErrorFormat(
                            "Failed to fix the offset out of range exception on topic:{0} partition:{1}.  Polling will continue.  Exception={2}",
        ...
        protected abstract void ConsumeMessage(Message message, CancellationToken cancellationToken);
    }
}
